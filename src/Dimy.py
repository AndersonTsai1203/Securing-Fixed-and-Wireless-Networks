...
DBF_MAX_SIZE = 6  # keep max 6 DBFs
...
class DimyNode:
    ...
    def generate_ephemeral_id(self):  ### Task 1
        ...
        self.ephemeral_id_hash = hashlib.sha256(bytes.fromhex(self.ephemeral_id)).hexdigest() # Segment 1
    ...
    def prepare_share_ephemeral_id(self):  ### Task 2
        ...
        print(f"Secret share in hexdecimal: {self.secret_shares}") # Segment 2
        ...
        while True:
            for index in range(len(self.secret_shares)):
                ...
                if probability < 0.5:
                    ...
                    print(f"Broadcasted message: {self.secret_shares[index]}") # Segment 3A
    ...
    def receive_secret_shares(self):  ### Task 4 - combine part a, b, c
        ...
        while True:
            ...
            if received_secret_hash == self.ephemeral_id_hash:
                ...
                print(f"Received secret share: {received_secret_share}") # Segment 3B
        ...
        for hash, shares in hash_counts.items():
            if len(shares) >= 3:
                ...
                if max(timestamps) - min(timestamps) >= 9:
                    ...
                    print(f"number of received shares: {len(self.received_shares)} - Ready to Reconstruct EphID...") # Segment 3C
    ...
    def reconstruct_ephemeral_id(self):  ### Task 4 - part c
        ...
        print("Attempting to reconstruct EphID from received shares...") # Segment 4A
        ...
        if len(self.received_shares) >= K:
            ...
            print(f"Reconstructed EphID: {self.reconstructed_ephemeral_id}") # Segment 4A
            if reconstructed_ephid_hash == self.received_hash:
                print(f"Verification successful") # Segment 4B
                print(f"reconstructed EphID hash: {reconstructed_ephid_hash} == received EphID hash: {self.received_hash}") # Segment 4B
    ...
    def perform_ecdh(self):  ### Task 5
        ...
        print(f"Generated EncID: {self.encounter_id.hex()}") # Segment 5A
    ...
    def create_dbf(self):  ### Task 6
        if self.first:
            ...
            if len(self.dbf_list) <= DBF_MAX_SIZE:
                self.dbf_list.append(self.dbf)
                self.create_qbf()
                self.dbf = BloomFilter(BLOOM_FILTER_SIZE, BLOOM_FILTER_HASHES)
                self.bloom_count = 0
                print("New DBF created after 90 seconds.") # Segment 7B
                print(f"Number of DBF created: {len(self.dbf_list)}") # Segment 7B

    def add_encounter_id(self):  ### Task 6
        print("Encoding EncID into DBF...") # Segment 6
        ...
        self.dbf.bits_non_zero()  # Segment 7A
        # Print number of encounter IDs that have been encoded in bloom filter
        self.bloom_count += 1
        print(f"Number of EncID encoded in DBF: {self.bloom_count}")  # Segment 7A
        # Delete Encounter ID
        self.encounter_id = None
        print("EncID deleted.") # Segment 6
    ...
    def create_qbf(self): ## Task 8
        ...
        if self.can_create_new_qbf():
            ...
            print(f"New QBF created.") # Segment 8
    ...
    def send_qbf_to_server(self):  ## Task 9
        ...
        print(f"QBF sent to server.") # Segment 10A

        response = self.tcp_socket.recv(1024)
        print(f"Server response for QBF: {response.decode()}") # Segment 10B

    def create_and_send_qbf(self):  ## Task 9
        if self.covid_positive:
            print("Covid positive. QBFs will cease to be sent.") # Segment 9
            return
        self.create_qbf()
    ...
    def create_cbf(self):  ## Task 10
        ...
        print(f"New CBF created.") # Segment 9
    ...
    def send_cbf_to_server(self):  ## Task 10
        ...
        response = self.tcp_socket.recv(1024)
        print(f"Server response for CBF: {response.decode()}") # Segment 9
